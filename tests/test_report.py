...
def test_formats_worst_first_table_with_missing_scores_last() -> None:
    modules = [
        ...
        (
            "other",
            [
                FunctionMetrics("safe_second", 1, 2, 1, 100.0, 1.0),
                FunctionMetrics("medium", 4, 7, 5, 50.0, 8.125),
            ],
        ),
    ]

    assert format_text_report(modules) == (
        "Function          Module  CC   Cov%  CRAP\n"
        "----------------  ------  --  -----  ----\n"
        "risky             sample   5    0.0  30.0\n"
        "medium            other    5   50.0   8.1\n"
        "safe_first        sample   1  100.0   1.0\n"
        "safe_second       other    1  100.0   1.0\n"
